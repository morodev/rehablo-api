...
import { scopeWhere, getUserId } from '../../../middleware/rbac.js';
...
async function writePayment(req: Request, res: Response, cumulative: boolean) {
    ...
    try {
        const result = await sequelize.transaction(async transaction => {
            ...
            const target = cumulative && requestedStatus === 'unpaid' ? 0
            ...
            if (!cumulative && amount <= 0) fail(400, 'L’importo deve essere almeno 0,01 euro');
        ...
        return sendSuccessResponse(res, cumulative ? 200 : 201, cumulative ? result.agendaEvent : result, 'Incasso registrato');
    } catch (error) {
...
export const listAppointmentPayments = asyncHandler(async (req, res) => {
    ...
    const price = (await appointmentPricesByEvent(schema, [event.get({ plain: true })])).get(event.id)!;
